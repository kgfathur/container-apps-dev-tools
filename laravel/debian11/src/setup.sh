...
WWW_ROOT=${WWW_ROOT:-/usr/share/nginx/html/public}
...
echo "Setup: repo"
apt-get update
apt-get install -y --no-install-recommends curl ca-certificates gnupg2 debian-archive-keyring git unzip
...
gpg --dry-run --quiet --no-keyring --import --import-options import-show ${APT_KEYRING_PATH}
echo "deb [signed-by=${APT_KEYRING_PATH}] https://deb.nodesource.com/node_${NODE_VERSION}.x ${OS_CODENAME} main
# deb-src [signed-by=${APT_KEYRING_PATH}] https://deb.nodesource.com/node_${NODE_VERSION}.x ${OS_CODENAME} main" | \
tee /etc/apt/sources.list.d/nodesource.list

APT_KEYRING_PATH=/etc/apt/trusted.gpg.d/yarnkey.gpg
curl -sL https://dl.yarnpkg.com/${OS_DISTRO}/pubkey.gpg | gpg  -v --yes --dearmor -o ${APT_KEYRING_PATH}
gpg --dry-run --quiet --no-keyring --import --import-options import-show ${APT_KEYRING_PATH}
echo "deb [signed-by=${APT_KEYRING_PATH}] \
https://dl.yarnpkg.com/${OS_DISTRO} stable main" | \
...
cp -vrf /src/app/* ${WWW_ROOT}/public/
...
sed -i "s|\(root\s\+\).*;$|\1${WWW_ROOT}/public;|" /etc/nginx/conf.d/default.conf
